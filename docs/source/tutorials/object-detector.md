...
meta_description: Siali Label tutorial for OpenMMLab Image object detector or MMDetection
...
2. Run Siali Label, then go to the **Machine Learning** page in the project settings. Click **Add Model**, then paste the selected ML backend URL, by default `http://localhost:9090`.
...
## Run ML backend on the same machine as Siali Label

If you run the ML backend on the same machine as Siali Label, you can upload images directly to Siali Label or specify them as remote URLs. 

#### Images are uploaded in Siali Label UI
   ```bash
...
## Run ML backend server on the different machine as Siali Label

When running the ML backend on a separate server instance and connecting it to Siali Label with a remote hostname URL, you must remember that your ML backend server must get the full image URLs.
In this case, you must provide the Siali Label hostname before running ML backend:
