---
title: Troubleshoot Siali Label
...
section: "Get started"
meta_title: Troubleshoot Siali Label
meta_description: Troubleshoot common issuesTroubleshoot machine learning with Siali Label configuration and performance so that you can return to your machine learning and data science projects.
---

If you encounter an issue using Siali Label, use this page to troubleshoot it. 
...
After starting Siali Label and opening a project, you see a blank page. Several possible issues could be the cause.

### Cause: Host not recognized

If you specify a host without a protocol such as `http://` or `https://` when starting Siali Label, Siali Label can fail to locate the correct files to load the project page.

<div class="opensource-only">

To resolve this issue, update the host specified as an environment variable or when starting Siali Label. See [Start Siali Label](start.html)

</div>

<div class="enterprise-only">

To resolve this issue, update the host specified as an environment variable or when starting Siali Label. Check LABEL_STUDIO_HOST environment variable.
...
If you want to upload a large volume of data (thousands of items), consider doing that at a time when people are not labeling or use a different database backend such as PostgreSQL or Redis. You can run Docker Compose from the root directory of Siali Label to use PostgreSQL: `docker-compose up -d`, or see [Sync data from cloud or database storage](storage.html). 
...
- If you use Amazon S3 with Siali Label, see [Troubleshoot CORS and access problems](storage.html#Troubleshoot-CORS-and-access-problems).
- If you use Google Cloud Storage with Siali Label, see [Troubleshoot CORS and access problems](storage.html#Troubleshoot-CORS-and-access-problems).
...
If the offsets for exported HTML labels don't match your expected output, such as with HTML named entity recognition (NER) tasks, the most common reason why is due to HTML minification. When you upload HTML files to Siali Label for labeling, the HTML is minified to remove whitespace. When you annotate those tasks, the offsets for the labels apply to the minified version of the HTML, rather than the original unmodified HTML files. 
...
If you want to correct existing annotations, you can minify your source HTML files in the same way that Siali Label does. The minification is performed with the following script:
