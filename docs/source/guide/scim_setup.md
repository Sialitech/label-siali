---
title: Set up SCIM2 for Siali Label
short: Set up SCIM2
tier: enterprise
type: guide
order: 113
order_enterprise: 140
meta_title: System for Cross-domain Identity Management (SCIM) for Siali Label Enterprise
meta_description: Siali Label Enterprise documentation for setting up SCIM2
section: "Install"

---

System for Cross-domain Identity Management (SCIM) is a popular protocol to manage access for services and applications across an organization. This guide helps to set up SCIM integration to manage access to Siali Label Enterprise in your organization. 

Using SCIM provider, you can manage access to Siali Label Enterprise workspaces, grant roles to individual users and groups.
...
To manage access to Siali Label Enterprise, add the application to your SCIM provider (Okta). Okta uses OAuth token to interact with REST API endpoints of the application to provision and deprovision access.

### Add Siali Label Enterprise (if not complete)

1. Navigate to **Applications → Applications** in Okta. Click  **Create App Integration**. 
2. Select **SAML 2.0**. Fill App name (for example, _Siali Label Enterprise_).
3. On the next step **Configure SAML** set up SAML integration following the instructions to [Set up SSO guide](auth_setup.html).
4. Make sure Siali Label Enterprise appears in the list of active applications.

### Enable SCIM provisioning

1. Navigate to **Applications → Applications** in Okta. 
2. Select **Siali Label Enterprise**.
...
Fill in the fields:
 - **SCIM connector base URL**: https://{LABEL_STUDIO_BASE_URL}/scim/v2/ where `{LABEL_STUDIO_BASE_URL}` is the base URL of your Siali Label Enterprise instance.
 - **Unique identifier field for users**: Leave `userName`. Siali Label Enterprise uses email as user identifier in this field.
...
3. Select the people you would like to be added to Siali Label Enterprise.
4. Click **Done**.

After you click **Done**, Okta will send the requests to create users accordingly in the Siali Label Enterprise.
...
The most convenient way to manage access to the application is via groups. You can assign Siali Label to groups and manage the groups in Okta. The changes will be propagated to the application.

### Set up group mapping

1. In Siali Label open SCIM settings (**Organization → SCIM**).
...
After saving the group assignment, the update will be queued and sent to Siali Label. 

!!! note
    Alternatively, you can push the changes immediately to Siali Label.
...
3. Find the group you would like to sync to Siali Label. 
