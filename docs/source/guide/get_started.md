...
order_enterprise: 99
section: "Get started"
meta_title: Overview of Siali Label
meta_description: Get started with Siali Label by creating projects to label and annotate data for machine learning and data science models.
---


## What is Siali Label?

Siali Label is an open source data labeling tool for labeling and exploring multiple types of data. It allows you to do the following:
- Perform different types of labeling with many data formats. 

<div class="enterprise-only">

- Use [Siali Label Enterprise as a cloud offering](https://heartex.com/product). 

</div>

- Integrate Siali Label with machine learning models to supply predictions for labels (pre-labels), or perform continuous active learning. See [Set up machine learning with your labeling process](ml.html). 

Siali Label is also available in Enterprise and Cloud editions with additional features. For more information, see the [Siali Label features](https://labelstud.io/guide/label_studio_compare.html) page.
...
## Quick start
> **NOTE:** For Siali Label 1.7.2 you have to to confirm the dependencies `libpq-dev` and `python3-dev` are installed. As an example, to install on a Debian-based distro:
> ```bash
> sudo apt install libpq-dev python3-dev
> ```

1. Install Siali Label:
```bash
...
label-studio start
```
3. Open the Siali Label UI at http://localhost:8080. 
...
When you upload data to Siali Label, each item in the dataset becomes a labeling task. The following table describes some terms you might encounter as you use Siali Label.
...
| Term | Description |
| --- | --- |
| Dataset | What you import into Siali Label, comprised of individual items, or labeling tasks. |
...
| Labels | What you add to each region while labeling a task in Siali Label. |
| Relation | A defined relationship between two labeled regions. |
| Result | A label applied to a specific region as stored in an annotation or prediction. See [Siali Label JSON format of annotated tasks](export.html#Label-Studio-JSON-format-of-annotated-tasks). |
| Annotations | The output of a labeling task. Previously called "completions". |
| Predictions, <br> Pre-annotations | Annotations in Siali Label format that machine learning models create for an unlabeled dataset. See [import pre-annotations](predictions.html) |
...
Start and finish a labeling project with Siali Label by following these steps:

<div class="opensource-only">

1. [Install Siali Label](install.html).
2. [Start Siali Label](start.html).
3. [Create accounts for Siali Label](signup.html). Create an account to manage and set up labeling projects.
...
1. [Create accounts for Siali Label](manage_users.html#Signup). Create an account to manage and set up labeling projects.
2. [Restrict access to the project](manage_users.html). Set up role-based access control. Only available in Siali Label Enterprise Edition.
...
7. [Review the annotated tasks](quality.html). Only available in Siali Label Enterprise Edition.
8. [Export the labeled data or the annotations](export.html).
...
You can use any of the Siali Label components in your own tools, or customize them to suit your needs. Before customizing Siali Label extensively, you might want to review Siali Label Enterprise Edition to see if it already contains the relevant functionality you want to build. See [Siali Label Features](https://labelstud.io/guide/label_studio_compare.html) for more.

The component parts of Siali Label are available as modular extensible packages that you can integrate into your existing machine learning processes and tools. 

| Module | Technology | Description |
| --- | --- | --- | 
| [Siali Label Backend](https://github.com/heartexlabs/label-studio/) | Python and [Django](https://www.djangoproject.com/) | Use to perform data labeling. | 
| [Siali Label Frontend](https://github.com/heartexlabs/label-studio-frontend) | JavaScript web app using [React](https://reactjs.org/) and [MST](https://github.com/mobxjs/mobx-state-tree) | Perform data labeling in a user interface. |
...
## Information collected by Siali Label

Siali Label collects anonymous usage statistics about the number of page visits and data types being used in labeling configurations that you set up. No sensitive information is included in the information we collect. The information we collect helps us improve the experience of labeling data in Siali Label and helps us plan future data types and labeling configurations to support. 
