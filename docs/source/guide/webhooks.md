---
title: Set up webhooks in Siali Label
short: Webhook setup
type: guide
tier: all
order: 216
order_enterprise: 123
meta_title: Configure Webhooks in Siali Label
meta_description: Siali Label documentation for setting up and configuring webhooks to integrate Siali Label with your machine learning pipeline.
section: "Integration and Development"

---

Webhooks in Siali Label let you set up integrations that subscribe to certain events that occur inside Siali Label. When an event is triggered, Siali Label sends an HTTP POST request to the configured webhook URL. For example:
```json
{
    "action": "TASK_CREATED",
    "tasks": [
...
## What to use Siali Label webhooks for 

Some events that happen in Siali Label happen in response to an API call. For those, the API response is sufficient to know that the action has occurred. But other events happen inside the Siali Label UI, such as project changes and task annotation activities. To find out immediately when those changes occur, you can use webhooks to notify your application or machine learning pipeline to take specific actions. 
...
## Available Siali Label webhooks
Siali Label makes two main types of events available to integrate with webhooks: project-level task events and organization events.
...
To integrate with webhooks in Siali Label, you must do the following:
1. Create a webhook endpoint or URL as part of your application or machine learning model pipeline.
2. Make sure the webhook endpoint or URL is accessible to your Siali Label instance and can accept HTTP POST requests.
3. [Add the webhook to Siali Label](webhooks.html#Add-a-new-webhook-in-Label-Studio).

Set up your webhook endpoint to read the event payload from Siali Label and take action based on the event.

## Add a new webhook in Siali Label

Set up a webhook URL in Siali Label and associate it with one or more event triggers. You can set up a webhook URL in the Siali Label UI, or using the API. You can configure as many webhook connections as you want, but too many might overwhelm your instance. 

### Add a webhook in Siali Label UI

Add a webhook URL to Siali Label. The webhook URL must be set up to accept HTTP POST requests.

1. In the Siali Label UI, open the project that you want to associate with a webhook URL.
...
8. (Optional) Select whether to send an event for all actions in Siali Label supported by webhooks, or specific events. By default, events are sent for all actions. For example, select the `Annotation created` event. 
9. Save the webhook.

### Add a webhook using the Siali Label API
...
If the webhook URL is inaccessible by Siali Label, you can see this in a traceback in the logs. 

Siali Label does not retry webhook connections that fail. You can see successful webhook deliveries in the logs in DEBUG mode. 
