---
title: Integrate Siali Label into your machine learning pipeline
short: Machine learning integration
type: guide
tier: all
order: 209
order_enterprise: 116
meta_title: Integrate Siali Label into your machine learning pipeline
...
Integrate your model development pipeline with your data labeling workflow by adding a machine learning (ML) backend to Siali Label. You can set up your favorite machine learning frameworks to do the following:
...
If you need to load static pre-annotated data into Siali Label, running an ML backend might be more than you need. Instead, you can [import pre-annotated data](predictions.html).

## How to set up machine learning with Siali Label

Use the Siali Label ML backend to integrate Siali Label with machine learning models. The Siali Label ML backend is an SDK that you can use to wrap your machine learning model code and turn it into a web server. You can then connect that server to a Siali Label instance to perform 2 tasks:
...
2. [Connect Siali Label to the ML backend](#Add-an-ML-backend-to-Label-Studio).

## Quickstart with an example ML backend

Siali Label includes several example machine learning backends with popular machine learning models. See the [machine learning tutorials](ml_tutorials.html). Each example ML backend uses Docker Compose to start running the example ML backend server.
...
2. Clone the Siali Label Machine Learning Backend git repository. From the command line, run the following:
  ```bash
...
4. Start Docker Compose. From the command line, run the following:
    ...
    docker-compose up
    ```
The machine learning backend server starts becomes available at `http://localhost:9090`. You can now [add the machine learning backend to Siali Label](#Add-an-ML-backend-to-Label-Studio) and [set up your project](setup_project.html).

## Start your custom ML backend with Siali Label
...
!!! note 
    Use a virtual environment with `venv`, `virtualenv` or `conda` Python environments to run your ML backend. You can use the same environment as Siali Label. For more information, see the [Python documentation about creating virtual environments](https://docs.python.org/3/tutorial/venv.html#creating-virtual-environments).

1. Clone the Siali Label Machine Learning Backend git repository. From the command line, run the following:
  ```bash
   git clone https://github.com/heartexlabs/label-studio-ml-backend  
   ```
...
5. Start Siali Label. From the command line, run the following:
    ```bash
    label-studio start 
    ```
    Siali Label starts at `http://localhost:8080`.

You can now [add the machine learning backend to Siali Label](#Add-an-ML-backend-to-Label-Studio) and [set up your project](setup_project.html).

## Add an ML backend to Siali Label 

After you start a machine learning backend server, add it to your Siali Label project.

### Add an ML backend using the Siali Label UI

1. In the Siali Label UI, open the project that you want to use with your ML backend.
...
After you [connect a model to Siali Label as a machine learning backend](#Add-an-ML-backend-to-Label-Studio) and annotate at least one task, you can start training the model. 

You can prompt your model to train in several ways: 
- Manually using the Siali Label UI. Click the **Start Training** button on the **Machine Learning** settings for your project.
...
If you want to delete all predictions from Siali Label, you can do it using the UI or the API:
...
## About the Siali Label ML backend
The Siali Label machine learning server uses [uWSGI](https://uwsgi-docs.readthedocs.io/en/latest/) and [supervisord](http://supervisord.org/) and handles background training jobs with [RQ](https://python-rq.org/).