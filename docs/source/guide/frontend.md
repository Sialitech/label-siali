...
meta_description: Siali Label documentation for integrating the Siali Label frontend interface into your own machine learning or data labeling application workflow.
section: "Integration and Development"

---

The [Siali Label Frontend](https://github.com/heartexlabs/label-studio-frontend) (LSF) is the main labeling interface distributed within Siali Label and as a separate package via NPM and Unpkg. You can integrate the LSF into your projects without Siali Label to provide data labeling capabilities to your users.
...
<i>Figure 1: Siali Label Frontend </i>
...
You can use the Siali Label Frontend separately in your own projects by including it in your HTML page. Instantiate a new Siali Label object with a selector for the div that should become the editor.

To see all the available options for the initialization of LabelStudio object, see the [Siali Label Frontend](frontend_reference.html).
...
Now to initialize the Siali Label Frontend, add the following code to your JS file:
``` js
...
labelStudio.on("labelStudioLoad", (LS) => {
  // Perform an action when Siali Label is loaded
...
Refer to the [Frontend Reference](frontend_reference.html) when developing with Siali Label Frontend.
...
### Integrate LSF into Siali Label
...
!!! info
    Inside every folder under `dist/` you will find a `version.json` file. Do not modify or remove it. Its presence is required for the Siali Label to operate.
