...
meta_description: Troubleshoot Siali Label connections with machine learning frameworks using the Siali Label ML backend SDK.
section: "Machine learning"

---

After you [set up machine learning with Siali Label](ml.html) or [create your own machine learning backend](ml_create.html) to use with Siali Label, you can troubleshoot any issues you encounter by reviewing the possible causes on this page.


## Troubleshoot by reviewing the ML server logs

You can investigate most problems using the server console log. The machine learning backend runs as a separate server from Siali Label, so make sure you check the correct server console logs while troubleshooting. To see more detailed logs, start the ML backend server with the `--debug` option. 
...
## Siali Label default timeout settings for ML server requests
Siali Label has default timeouts for all types of requests to ML server.   

Siali Label has several different requests to ML server:
...
3. Predict - prediction request when Siali Label gets predictions from ML backend (env variable ML_TIMEOUT_PREDICT)
...
You can adjust the timeout by setting an environment variables for each request or modify in Siali Label variables. These are the variables section in Siali Label (in seconds):
...
## I launched the ML backend, but it appears as **Disconnected** after adding it in the Siali Label UI
...
- Confirm that your project's label configuration matches the output produced by your ML backend. For example, use the Choices tag to create a class of predictions for text. See more [Siali Label tags](/tags). 
...
Because the ML backend and Siali Label are different services, the assets (images, audio, etc.) that you label must be hosted and be accessible with URLs by the machine learning backend, otherwise it might fail to create predictions.

## I get a validation error when adding the ML backend

If you get a validation error when adding the ML backend URL to your Siali Label project, check the following:
...
- Is your machine learning backend available from your Siali Label instance? It must be available to the instance running Siali Label.
  
If you're running Siali Label in Docker, you must run the machine learning backend inside the same Docker container, or otherwise make it available to the Docker container running Siali Label. You can use the `docker exec` command to run commands inside the Docker container, or use `docker exec -it <container_id> /bin/sh` to start a shell in the context of the container. See the [docker exec documentation](https://docs.docker.com/engine/reference/commandline/exec/). 
